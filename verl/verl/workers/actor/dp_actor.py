...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(
        ...
        with torch.autocast(device_type=self.device_name, dtype=self.param_dtype):
            ...
            # Register forward hook for hidden state extraction
            if return_hidden_states:

                hook = HiddenStateHook()
                # Find the target layer to hook.  hidden_states_layer_idx=1
                # means the last layer (last_layer_idx=1), 2 means second-to-last, etc.
                # We hook on the transformer decoder layer's output (after
                # self-attn + MLP + residual), which gives the hidden states
                # that feed into the next layer (or final norm).
                base_model = getattr(self.actor_module, "module", self.actor_module)
                # Navigate the model hierarchy: model.model.layers for Qwen/Llama
                inner = getattr(base_model, "model", base_model)
                layers = getattr(inner, "layers", None)
                if layers is not None and len(layers) >= hidden_states_layer_idx:
                    target_layer = layers[len(layers) - hidden_states_layer_idx]
                    handle = target_layer.register_forward_hook(hook)
                else:
                    # Fallback: hook on final norm
                    norm = getattr(inner, "norm", None)
                    if norm is not None:
                        handle = norm.register_forward_hook(hook)
                    else:
                        logger.warning("Cannot find target layer for hidden state hook; skipping.")
                        hook = None
            ...
            # Extract hidden states from hook if requested
            if return_hidden_states and hook is not None and hook.hidden_states is not None:
                captured_hs = hook.hidden_states  # from decoder layer: (total_nnz, hidden) or (bs, seq, hidden)
                if self.use_remove_padding:
                    # The hook captured rmpad output — shape depends on the layer:
                    # decoder layer output is a tuple; the hook captures the
                    # output tensor which has shape (1, total_nnz, hidden) or
                    # (total_nnz, hidden).  Re-pad to (bs, seqlen, hidden).
                    if captured_hs.dim() == 3 and captured_hs.shape[0] == 1:
                        captured_hs = captured_hs.squeeze(0)  # (total_nnz, hidden)
                    if captured_hs.dim() == 2:
                        full_hs = pad_input(
                            hidden_states=captured_hs,
                            indices=indices,
                            batch=batch_size,
                            seqlen=seqlen,
                        )  # (bs, seqlen, hidden)
                    else:
                        full_hs = captured_hs  # already (bs, seqlen, hidden)
                else:
                    full_hs = captured_hs  # (bs, seqlen, hidden)
                outputs["hidden_states"] = full_hs.detach().cpu()
                hook.clear()
    ...
    def compute_log_prob(
        self, data: DataProto, calculate_entropy: bool = False, return_hidden_states: bool = False
    ) -> dict[str, torch.Tensor]:
        ...
        Args:
            ...
            return_hidden_states: if True, also return hidden states for probe-based confidence.
        ...
        Returns:
            dict[str, torch.Tensor]: a dict containing keys
                ...
                - ``hidden_states`` (optional): tensor of shape [batch_size, seq_len, hidden_size]. On CPU.
        """
        calculate_sum_pi_squared = self.config.get("calculate_sum_pi_squared", False)
        ...
        hidden_states_layer_idx = data.meta_info.get("hidden_states_layer_idx", 1)
        for micro_batch in micro_batches:
            ...
            if return_hidden_states and "hidden_states" in outputs:
                hidden_states_lst.append(outputs["hidden_states"])
        ...
        if use_dynamic_bsz:
            ...
            if hidden_states is not None:
                revert_indices = torch.tensor(
                    [idx for sublist in batch_idx_list for idx in sublist], dtype=torch.long
                )
                revert_indices = torch.argsort(revert_indices)
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    # c2gspg loss needs binary correctness labels for its BCE calibration term
                    extra_loss_kwargs = {}
                    if loss_mode == "c2gspg":
                        extra_loss_kwargs["c2_binary_reward"] = model_inputs.get("c2_binary_reward", None)
