...
def get_zmq_handle(device_uuid: str) -> str:
    """Build a job-scoped IPC handle to avoid cross-job socket collisions."""
    namespace = os.getenv("VERL_ZMQ_NAMESPACE") or os.getenv("SLURM_JOB_ID") or f"uid{os.getuid()}"
    safe_namespace = "".join(ch if (ch.isalnum() or ch in "._-") else "_" for ch in str(namespace))
    return f"ipc:///tmp/rl-colocate-zmq-{device_uuid}-{safe_namespace}.sock"
