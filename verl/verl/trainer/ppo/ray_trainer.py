...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        # Optional C2GSPG-specific tensors.
        if "old_sequence_confidence" in data.batch:
            adv_kwargs["old_sequence_confidence"] = data.batch["old_sequence_confidence"]
        if "c2_binary_reward" in data.batch:
            adv_kwargs["c2_binary_reward"] = data.batch["c2_binary_reward"]
...
class RayPPOTrainer:
    ...
    def _dump_generations(self, inputs, outputs, gts, scores, reward_extra_infos_dict, dump_path):
        ...
        lines = []
        for i in range(n):
            entry = {k: convert_to_serializable(v[i]) for k, v in base_data.items()}
