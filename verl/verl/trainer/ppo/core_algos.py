...
@register_adv_est("c2gspg")
def compute_c2gspg_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    old_sequence_confidence: Optional[torch.Tensor] = None,
    c2_binary_reward: Optional[torch.Tensor] = None,
    epsilon: float = 1e-6,
    config: Optional[AlgoConfig] = None,
    **kwargs,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Compute C2GSPG outcome advantage.

    Implements:
        A_i = (r_i - m_group) / (1 - c_old_i)

    where r_i in {0,1} is a per-sequence binary label, m_group is the
    group mean (grouped by uid / prompt index), and c_old_i is sequence-level
    confidence under the old policy.
    """
    with torch.no_grad():
        scores = token_level_rewards.sum(dim=-1)
        ...
        # Use explicit binary labels when provided, else fallback to score sign.
        if c2_binary_reward is None:
            binary_reward = (scores > 0).to(dtype)
        else:
            binary_reward = c2_binary_reward.reshape(-1).to(device=device, dtype=dtype)

        # Confidence defaults to zero when unavailable so denominator is ~1.
        if old_sequence_confidence is None:
            old_conf = torch.zeros_like(binary_reward)
        else:
            old_conf = old_sequence_confidence.reshape(-1).to(device=device, dtype=dtype)

        conf_clip_eps = epsilon
        denom_eps = epsilon
        if config is not None:
            conf_clip_eps = float(config.get("c2gspg_confidence_clip_eps", epsilon))
            denom_eps = float(config.get("c2gspg_denominator_eps", epsilon))

        old_conf = torch.clamp(old_conf, min=0.0, max=1.0 - conf_clip_eps)

        # Compute per-group mean reward.
        if index is None:
            centered_reward = binary_reward - binary_reward.mean()
        else:
            id2reward = defaultdict(list)
            id2mean = {}
            for i in range(bsz):
                id2reward[index[i]].append(binary_reward[i])
            for idx in id2reward:
                id2mean[idx] = torch.mean(torch.stack(id2reward[idx]))

            centered_reward = torch.empty_like(binary_reward)
            for i in range(bsz):
                centered_reward[i] = binary_reward[i] - id2mean[index[i]]

        scalar_advantages = centered_reward / torch.clamp(1.0 - old_conf, min=denom_eps)
...
@register_policy_loss("c2gspg")
def compute_policy_loss_c2gspg(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    config: Optional[ActorConfig] = None,
    rollout_is_weights: torch.Tensor | None = None,
    c2_binary_reward: torch.Tensor | None = None,
) -> tuple[torch.Tensor, dict[str, Any]]:
    """C2GSPG policy loss: PPO clipped objective + BCE confidence calibration regularizer.

    Implements eq. (6) from the C2GSPG paper:
        J = Ã_i · log c_{θ,i}  +  β · [r_i log c_{θ,i} + (1-r_i) log(1-c_{θ,i})]

    The PPO clipped objective approximates the policy gradient term Ã_i · log c_{θ,i}.
    The BCE regularizer pushes new-policy confidence toward binary correctness labels.

    BCE is only applied when c2_binary_reward is provided and c2gspg_bce_beta > 0.
    Otherwise this is identical to the vanilla PPO loss.
    """
    assert config is not None
    assert not isinstance(config, AlgoConfig)

    # --- Standard PPO clipped objective (same as vanilla) ---
    ...
    # --- BCE confidence calibration regularizer ---
    bce_beta = float(config.policy_loss.get("c2gspg_bce_beta", 0.0)) if config.policy_loss is not None else 0.0
    if bce_beta > 0.0 and c2_binary_reward is not None:
        r = c2_binary_reward.reshape(-1).to(dtype=log_prob.dtype, device=log_prob.device)
        # New-policy sequence confidence: c_{θ,i} = exp(mean token log-prob)
        response_mask_f = response_mask.to(dtype=log_prob.dtype)
        token_count = torch.clamp(response_mask_f.sum(dim=-1), min=1.0)
        new_seq_log_conf = (log_prob * response_mask_f).sum(dim=-1) / token_count  # (bsz,)
        # Clamp log-confidence away from 0/-inf for BCE stability
        eps = 1e-6
        new_seq_conf = torch.clamp(torch.exp(new_seq_log_conf), min=eps, max=1.0 - eps)

        bce_per_seq = -(r * torch.log(new_seq_conf) + (1 - r) * torch.log(1 - new_seq_conf))  # (bsz,)
        # Broadcast to token level so agg_loss handles distributed normalisation consistently
        bce_token_mat = bce_per_seq.unsqueeze(-1).expand_as(response_mask_f) * response_mask_f
        bce_loss = agg_loss(
            loss_mat=bce_token_mat, loss_mask=response_mask, loss_agg_mode=loss_agg_mode, **config.global_batch_info
        )

        pg_metrics["actor/c2gspg_bce_loss"] = bce_loss.detach().item()
        pg_metrics["actor/c2gspg_bce_beta"] = bce_beta
        total_loss = pg_loss + bce_beta * bce_loss
    else:
        total_loss = pg_loss
