...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    Returns:
        A dictionary of metrics including:
            ...
            - critic/advantages/mean, max, min, std: Statistics about advantages
...
def compute_solved_metrics(
    batch: DataProto,
    *,
    uid_key: str = "uid",
    score_key: str = "token_level_scores",
    correct_if_score_gt: float = 0.0,
) -> dict[str, Any]:
    """Compute per-batch solved-all / solved-none counts grouped by problem uid.

    "Solved" here means the sample's *sequence score* (sum of token-level scores)
    is strictly greater than ``correct_if_score_gt``.

    Returns two training metrics:
      - training/solved_all: #uids in batch where *all* group samples are solved
      - training/solved_none: #uids in batch where *none* of group samples are solved

    This is intended for any PO-style method (GRPO/DAPO, etc.) that runs
    through ray_trainer and uses uid-grouped rollouts.
    """

    if not hasattr(batch, "non_tensor_batch") or uid_key not in batch.non_tensor_batch:
        return {}

    uids = batch.non_tensor_batch[uid_key]
    # Make uids a stable numpy array (works for list, np.ndarray, torch.Tensor)
    try:
    ...
    is_correct = None
    if "forced_output_any_correct" in batch.non_tensor_batch:
        forced_output_any_correct = batch.non_tensor_batch["forced_output_any_correct"]
        try:
            is_correct = np.asarray(forced_output_any_correct).reshape(-1).astype(bool)
        except Exception:
            is_correct = np.array(list(forced_output_any_correct), dtype=bool)
        if is_correct.shape[0] != uids_np.shape[0]:
            is_correct = None

    if is_correct is None:
        if not hasattr(batch, "batch") or score_key not in batch.batch:
            return {}
        token_level_scores = batch.batch[score_key]
        ...
        is_correct = (seq_score > correct_if_score_gt).detach().cpu().numpy().astype(bool)
    ...
    return {
        "training/solved_all": int(solved_all),
        "training/solved_none": int(solved_none),
    }
...
def compute_rlcr_metrics(reward_extra_infos_dict: dict) -> dict[str, float]:
    """Compute probe-style metrics from RLCR verbalized confidence.

    When the reward function returns ``confidence`` and ``acc`` keys (i.e. the
    RLCR scorer), this mirrors the probe metrics logged during forced-output
    training, treating the verbalized confidence as the "prediction":

        probe/mean_pred          — mean verbalized confidence
        probe/binary_accuracy    — fraction where (conf > 0.5) matches acc
        probe/mean_conf_correct  — mean confidence on correct samples
        probe/mean_conf_incorrect — mean confidence on incorrect samples
        probe/std_conf_correct   — std  confidence on correct samples
        probe/std_conf_incorrect — std  confidence on incorrect samples

    Returns an empty dict if the required keys are absent.
    """
    if "confidence" not in reward_extra_infos_dict or "acc" not in reward_extra_infos_dict:
        return {}

    confidences = reward_extra_infos_dict["confidence"]
    accs = reward_extra_infos_dict["acc"]

    try:
        conf_arr = np.asarray(confidences, dtype=np.float32)
        acc_arr = np.asarray(accs, dtype=np.float32)
    except Exception:
        return {}

    if conf_arr.ndim != 1 or acc_arr.ndim != 1 or len(conf_arr) == 0:
        return {}

    pred_binary = (conf_arr > 0.5).astype(np.float32)
    target_binary = (acc_arr > 0.5).astype(np.float32)
    binary_accuracy = float((pred_binary == target_binary).mean())
    ...
    mean_correct, std_correct = _summary(conf_arr[correct_mask])
    mean_incorrect, std_incorrect = _summary(conf_arr[incorrect_mask])

    metrics = {
        "probe/mean_pred": float(conf_arr.mean()),
        "probe/binary_accuracy": binary_accuracy,
    ...
    if "answer_valid" in reward_extra_infos_dict:
        try:
            answer_valid_arr = np.asarray(reward_extra_infos_dict["answer_valid"], dtype=np.float32)
            if answer_valid_arr.ndim == 1 and len(answer_valid_arr) > 0:
                metrics["rlcr/answer_extraction_failure_rate"] = float(1.0 - answer_valid_arr.mean())
        except Exception:
            pass
    ...
    for data_source, uid2var2vals in data_src2uid2var2vals.items():
        ...
        for uid, var2vals in uid2var2vals.items():
            ...
            for var_name, var_vals in var2vals.items():
                ...
                # filter out None and string values, keep only numeric values
                var_vals = [v for v in var_vals if v is not None and isinstance(v, (int, float, bool))]
                if not var_vals:
                    continue
