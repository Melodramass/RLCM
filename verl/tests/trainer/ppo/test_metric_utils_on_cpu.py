...
class TestComputeDataMetrics(unittest.TestCase):
    ...
    def test_compute_data_metrics_with_critic(self):
        ...
        self.assertIn("critic/advantages/std", metrics)
...
class TestComputeSolvedMetrics(unittest.TestCase):
    def test_compute_solved_metrics_counts(self):
        batch = MagicMock()
        batch.batch = {
            # seq_score = sum(-1)
            "token_level_scores": torch.tensor(
                [
                    [1.0, 0.0],  # uid 1 correct
                    [1.0, 0.0],  # uid 1 correct  -> solved_all
                    [-1.0, 0.0],  # uid 2 incorrect
                    [1.0, 0.0],  # uid 2 correct   -> mixed
                    [0.0, 0.0],  # uid 3 incorrect
                    [0.0, 0.0],  # uid 3 incorrect -> solved_none
                ]
            ),
        }
        batch.non_tensor_batch = {
            "uid": np.asarray([1, 1, 2, 2, 3, 3]),
        }

        metrics = compute_solved_metrics(batch)
        self.assertEqual(metrics["training/solved_all"], 1)
        self.assertEqual(metrics["training/solved_none"], 1)

    def test_compute_solved_metrics_missing_uid(self):
        batch = MagicMock()
        batch.batch = {"token_level_scores": torch.tensor([[1.0]])}
        batch.non_tensor_batch = {}
        self.assertEqual(compute_solved_metrics(batch), {})
...
class TestComputeRlcrMetrics(unittest.TestCase):
    """Tests for the compute_rlcr_metrics function."""

    def test_missing_keys_returns_empty(self):
        """Returns empty dict when confidence/acc keys are absent."""
        self.assertEqual(compute_rlcr_metrics({}), {})
        self.assertEqual(compute_rlcr_metrics({"confidence": [0.8]}), {})
        self.assertEqual(compute_rlcr_metrics({"acc": [1]}), {})

    def test_all_correct_high_confidence(self):
        """All correct + high confidence: conf_correct high, binary_accuracy high."""
        result = compute_rlcr_metrics({
            "confidence": [0.9, 0.8, 0.7],
            "acc": [1, 1, 1],
        })
        self.assertIn("probe/mean_pred", result)
        self.assertIn("probe/binary_accuracy", result)
        self.assertIn("probe/mean_conf_correct", result)
        self.assertIn("probe/mean_conf_incorrect", result)
        self.assertIn("probe/std_conf_correct", result)
        self.assertIn("probe/std_conf_incorrect", result)

        self.assertAlmostEqual(result["probe/mean_pred"], (0.9 + 0.8 + 0.7) / 3, places=5)
        # All conf > 0.5 and all acc == 1 -> binary_accuracy = 1.0
        self.assertAlmostEqual(result["probe/binary_accuracy"], 1.0, places=5)
        # No incorrect samples
        self.assertTrue(np.isnan(result["probe/mean_conf_incorrect"]))
        self.assertTrue(np.isnan(result["probe/std_conf_incorrect"]))

    def test_all_incorrect_low_confidence(self):
        """All incorrect + low confidence: binary_accuracy high (predict incorrect correctly)."""
        result = compute_rlcr_metrics({
            "confidence": [0.1, 0.2, 0.3],
            "acc": [0, 0, 0],
        })
        # conf < 0.5 and acc == 0 -> binary_accuracy = 1.0
        self.assertAlmostEqual(result["probe/binary_accuracy"], 1.0, places=5)
        self.assertTrue(np.isnan(result["probe/mean_conf_correct"]))

    def test_mixed_correct_incorrect(self):
        """Mixed samples: check conf split by correctness."""
        result = compute_rlcr_metrics({
            "confidence": [0.9, 0.8, 0.2, 0.1],
            "acc":        [1,   1,   0,   0],
        })
        self.assertAlmostEqual(result["probe/mean_conf_correct"], (0.9 + 0.8) / 2, places=5)
        self.assertAlmostEqual(result["probe/mean_conf_incorrect"], (0.2 + 0.1) / 2, places=5)
        # binary_accuracy: all predictions match ground truth
        self.assertAlmostEqual(result["probe/binary_accuracy"], 1.0, places=5)

    def test_miscalibrated_confidence(self):
        """Low confidence on correct, high confidence on incorrect."""
        result = compute_rlcr_metrics({
            "confidence": [0.2, 0.9],
            "acc":        [1,   0],
        })
        # pred_binary=[0,1], target_binary=[1,0] -> accuracy = 0
        self.assertAlmostEqual(result["probe/binary_accuracy"], 0.0, places=5)
        self.assertAlmostEqual(result["probe/mean_conf_correct"], 0.2, places=5)
        self.assertAlmostEqual(result["probe/mean_conf_incorrect"], 0.9, places=5)

    def test_std_single_sample(self):
        """Std with single sample per bucket is 0."""
        result = compute_rlcr_metrics({
            "confidence": [0.8, 0.3],
            "acc":        [1,   0],
        })
        self.assertAlmostEqual(result["probe/std_conf_correct"], 0.0, places=5)
        self.assertAlmostEqual(result["probe/std_conf_incorrect"], 0.0, places=5)

    def test_answer_extraction_failure_rate(self):
        """answer_valid key produces failure rate metric."""
        result = compute_rlcr_metrics({
            "confidence":   [0.9, 0.8, 0.7, 0.6],
            "acc":          [1,   1,   0,   0],
            "answer_valid": [True, True, False, True],
        })
        # 1 out of 4 failed -> 0.25
        self.assertIn("rlcr/answer_extraction_failure_rate", result)
        self.assertAlmostEqual(result["rlcr/answer_extraction_failure_rate"], 0.25, places=5)

    def test_answer_extraction_failure_rate_absent(self):
        """No answer_valid key -> metric not logged."""
        result = compute_rlcr_metrics({
            "confidence": [0.9, 0.8],
            "acc":        [1,   0],
        })
        self.assertNotIn("rlcr/answer_extraction_failure_rate", result)
