#!/bin/bash
# =============================================================================
# Experiment e30 — GRPO + Forced Output + Probe (conf_diff_v3)
# =============================================================================
#
# GOAL
...
#
#   ── Probe reward (conf_diff_v3 mode, margin_corner=True) ──
...
#     Train BudgetProbeMLP for 16 steps (lr=1e-3) predicting mc_acc(bk) from
#     these hidden states.  Then infer probe confidence on the same states:
#
#       conf_correct   = {probe_conf(bk) : mc_acc(bk) == 1.0}  (all-correct budgets)
#       conf_incorrect = {probe_conf(bk) : mc_acc(bk) == 0.0}  (all-incorrect budgets)
#       (partial budgets 0 < mc_acc < 1 are EXCLUDED from conf_diff)
#
#       conf_diff = mean(conf_correct) - mean(conf_incorrect)
#                   (natural reward is NOT included in conf_diff computation)
#
#       Case weights (conf_diff_v3):
#         • Both classes present  → alpha_both * (mu_conf_correct - mu_conf_incorrect)
#         • All budgets correct   → alpha_only_correct * (mu_conf_correct - tau)
#         • All budgets incorrect → alpha_only_incorrect * (tau - mu_conf_incorrect)
#
#     probe_reward = probe_reward_weight × conf_diff  =  0.1 × conf_diff
#
#   ── FINAL GRPO reward ──
#     R = r_nat + 0.1 × conf_diff
#
#   Intuition: r_nat trains for correctness; conf_diff trains the model's hidden
#   states to be well-calibrated (high confidence where correct, low where not)
#   so the probe can predict budget-level correctness from the representation.
#
# KEY DIFFERENCES FROM RELATED EXPERIMENTS
#   • vs e17 / conf_diff_v1: uses plain conf_diff (not v1); corner case → 0 (not penalized).
#   • vs e19-..._0.5_true  : same conf_diff mode but margin_corner=True there,
#       meaning one-class traces do receive a corner reward signal.
#   • vs e13 / e14 (no probe): adds the probe MLP and conf_diff auxiliary reward.
#
# RUN
#   bash e30-grpo_forced_output_lead_bs32_conf_diff_v3.sh [--model PATH]
# =============================================================================
set -x
ray stop
...
RAY_DEDUP_LOGS=0 PYTHONUNBUFFERED=1 python3 -m verl.trainer.ppo.forced_output.main_forced_output_grpo \
    ...
    +forced_output.probe.reward_weight=0.1 \
    +forced_output.probe.reward_mode=conf_diff_v3 \
    +forced_output.probe.margin_corner=True \
    +forced_output.probe.conf_diff_v2_tau=0.0 \
    +forced_output.probe.conf_diff_v3_alpha_both=0.0 \
    +forced_output.probe.conf_diff_v3_alpha_only_correct=1.0 \
    +forced_output.probe.conf_diff_v3_alpha_only_incorrect=1.0 \
    +forced_output.probe.train_steps=16 \
