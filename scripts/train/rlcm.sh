#!/bin/bash
# =============================================================================
# Experiment e19 — GRPO + Forced Output + Probe (conf_diff, no corner cases)
# =============================================================================
#
# GOAL
...
#
#   ── Probe reward (conf_diff mode, margin_corner=False) ──
...
#
#       Corner cases (margin_corner=False = "no_corner"):
#         • All budgets correct  (conf_incorrect empty) → conf_diff = 0
#         • All budgets incorrect (conf_correct  empty) → conf_diff = 0
#
#     probe_reward = probe_reward_weight × conf_diff  =  0.3 × conf_diff
#
#   ── FINAL GRPO reward ──
#     R = r_nat + 0.3 × conf_diff
#
...
#
# RUN
#   bash e19-grpo_forced_output_lead_bs32_conf_diff_no_corner.sh [--model PATH]
# =============================================================================
set -x
ray stop
...
# W&B project / run name — override via env vars, e.g.:
#   WANDB_PROJECT=my-project RUN_NAME=my-run bash scripts/train/rlcm.sh
project_name="${WANDB_PROJECT:-anytime-reasoning}"
run_name="${RUN_NAME:-rlcm}"
...
RAY_DEDUP_LOGS=0 PYTHONUNBUFFERED=1 python3 -m verl.trainer.ppo.forced_output.main_forced_output_grpo \
    ...
    '+forced_output.budget_checkpoints=[0.25,0.5,0.75]' \
