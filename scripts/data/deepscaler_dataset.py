...
from rlcm.data.utils import load_dataset
from rlcm.data.dataset_types import TrainDataset, TestDataset
...
def make_map_fn(split: str):
    ...
    """
    def process_fn(example: Dict[str, Any], idx: int, data_source="") -> Optional[Dict[str, Any]]:
...
if __name__ == '__main__':
    ...
    for test_dataset, test_data_list in zip(test_datasets, test_datasets_data):
        ...
        for idx, example in enumerate(test_data_list):
            processed_example = process_fn(example, idx, data_source=dataset_name)
            if processed_example is not None:
